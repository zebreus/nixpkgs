...
let
  ...
  duplicateClangVersion = lib.lists.findFirst (clangVersion: (builtins.length (builtins.filter (otherClangVersion: (lib.versions.major clangVersion) == (lib.versions.major otherClangVersion)) passedClangVersions)) > 1) null passedClangVersions;
  passedGccVersions = builtins.map (gcc: gcc.version) gccFrontends;
  duplicateGccVersion = lib.lists.findFirst (gccVersion: (builtins.length (builtins.filter (otherGccVersion: (lib.versions.major gccVersion) == (lib.versions.major otherGccVersion)) passedGccVersions)) > 1) null passedGccVersions;
in
assert lib.assertMsg (duplicateClangVersion == null) "Multiple clang frontends with the same major version (${lib.versions.major duplicateClangVersion}) are not supported by bambu. You passed packages with the following versions: ${builtins.concatStringsSep " " (builtins.filter (clangVersion: (lib.versions.major clangVersion) == (lib.versions.major duplicateClangVersion)) passedClangVersions)}";

assert lib.assertMsg (duplicateGccVersion == null) "Multiple gcc frontends with the same major version (${lib.versions.major duplicateGccVersion}) are not supported by bambu. You passed packages with the following versions: ${builtins.concatStringsSep " " (builtins.filter (gccVersion: (lib.versions.major gccVersion) == (lib.versions.major duplicateGccVersion)) passedGccVersions)}";
